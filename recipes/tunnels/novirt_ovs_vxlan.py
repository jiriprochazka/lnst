...
h1_eth = h1.get_interface("if1")
h2_eth = h2.get_interface("if1")
...
for state in ["on", "off"]:
    h1.run("ethtool -K %s tx-udp_tnl-segmentation %s" % (h1_eth.get_devname(), state))
    h2.run("ethtool -K %s tx-udp_tnl-segmentation %s" % (h2_eth.get_devname(), state))
